...
func main() {
	...
	logger.Printf("configuration loaded: config=%s data_dir=%s geoip_database=%s reject_private_addresses=%t allow_update_without_old=%t trust_proxy_headers=%t log_raw_requests=%t",
		configPath, cfg.DataDir, cfg.GeoIPDatabase, cfg.RejectPrivateAddresses, cfg.AllowUpdateWithoutOld, cfg.TrustProxyHeaders, cfg.LogRawRequests)
